...
BOOL program_check_context(const ProgramContext *context) {
  ...
  if (ret) {
    ...
    if (position_x + w > framebuffer_get_width(fb) ||
        position_y + h > framebuffer_get_height(fb) || position_x < 0 ||
        position_y < 0) {
  ...
  if (ret) {
    int t =
        program_context_get_integer(context, "transparency", DEF_TRANSPARENCY);
    if (t < 0 || t > 100) {
...
int program_run(ProgramContext *context) {
  ...
  log_set_level(program_context_get_integer(context, "log-level", LOG_WARNING));
  ...
  if (error == NULL) {
    if (program_check_context(context)) {
      ...
      int transparency = program_context_get_integer(context, "transparency",
                                                     DEF_TRANSPARENCY);
      BOOL seconds = program_context_get_boolean(context, "seconds", FALSE);
      BOOL date = program_context_get_boolean(context, "date", FALSE);
