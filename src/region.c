...
/*==========================================================================
  ...
  elsewhere
*==========================================================================*/
void region_set_pixel_t(Region *self, int x, int y, BYTE r, BYTE g, BYTE b,
                        float t) {
  b = (BYTE)(t * (float)b);
  g = (BYTE)(t * (float)g);
  r = (BYTE)(t * (float)r);
...
*==========================================================================*/
void region_fill_rect(Region *self, int x1, int y1, int x2, int y2, BYTE r,
                      BYTE g, BYTE b) {
  LOG_IN
  if (x1 > x2) {
    int t = x1;
...
*==========================================================================*/
void region_draw_bitmap_char(Region *self, const BitmapFont *bf, char c, int x,
  ...
  if (x < self->w && y < self->h) {
    if (c < ' ' || c > '~')
      c = '?';
    int char_offset =
        (c - ' ') * bf->height * (bf->width / 8 + (bf->width % 8 ? 1 : 0));
...
void region_draw_bitmap_text(Region *self, const BitmapFont *bf,
                             const char *text, int x, int y, int r, int g,
                             int b) {
  LOG_IN
  int l = strlen(text);
  for (int i = 0; i < l; i++) {
...
*==========================================================================*/
void region_draw_line_one_pixel(Region *self, int x0, int y0, int x1, int y1,
...
/*==========================================================================
  ...
  a filled rectangle of width t between (x1, y1) and (x2, y2), center
